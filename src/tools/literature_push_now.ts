  ...
  return defineTool({
    ...
    async execute(args: PushNowInput): Promise<PushNowOutput> {
      ...
      const instructions = [
        ...
        `4. 语义排序（你负责，0~1 评分并记录 rationale）——必须 BATCH：一次（至多两次）LLM 调用对 deterministic Top 15 做批量语义评估，输出结构化评分表逐篇给出 relevance、learning_value、representativeness、novelty、stage_relevance_score（是否属于当前阶段）与 curriculum_value_score（对系统学习当前阶段是否核心/代表性/值得优先读——考虑 foundational importance、method centrality、learning value、representativeness、prerequisite suitability；过于具体的应用/单机构设计案例应给低分）。禁止逐篇发起独立 LLM 请求。性能目标：候选排序阶段（检索+语义排序）≤ 1~2 分钟，主要耗时留给最终论文全文精读；literature_record 时自报 llmCallCount（目标 1~2 次）。Fundamentals 阶段 curriculum_value 权重更高。`,
        ...
        `6b. 机构访问（Direct Publisher Access；Quality First, Access Second）——仅当全部 Top-K 候选公开全文 preflight 全失败后：按 agent 语义排名对达标候选**逐篇**调用 literature_fetch_pdf（传 pushId 与 allowInstitutional=true；严格低频，每次仅一篇）。流程：public/OA 链 → 失败 → DOI direct resolution → publisher article page（publisher_browser provider，独立持久浏览器 profile）→ PDF。终态处理：PDF_OK → 该论文 SELECTED（reason 注明机构访问，非 OA，仅私人文献库）；AUTH_REQUIRED（登录墙）→ 立即停止，用 literature_user_action(open) 注册待办（kind=publisher_login，五要素）并 record status=user_action_required、errorCode=AUTH_REQUIRED，提示用户运行 bin/dsh-literature-browser-login（headed 浏览器完成合法登录；绝不自动填写账号/密码/验证码）后 --resume 继续（不得记为 fulltext_unavailable，不产生 cooldown，不得为了省事跳过高质量 Rank1 去选低质量 OA Rank2）；ACCESS_DENIED（登录后明确无订阅/403）→ 才继续下一排名候选；PDF_NOT_FOUND → 继续下一排名候选；全部失败 → status=fulltext_unavailable。CARSI 为 legacy 且默认禁用，正常流程不调用。`,
        ...
        '9. literature_fetch_pdf（传 pushId）：下载选中论文 PDF（多源回退；机构访问见 6b）。',

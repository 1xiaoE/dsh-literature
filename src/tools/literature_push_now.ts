  ...
  return defineTool({
    ...
    async execute(args: PushNowInput): Promise<PushNowOutput> {
      ...
      const instructions = [
        ...
        `7. 知识缺口引导：priority knowledge goal = 本阶段第一个未覆盖 goal（当前由 stages.covered_goals 决定）。优先选择 priorityGoalMatch=true 的候选（预排序已加权），agent 的 curriculum 排序也应对 priority goal 匹配论文显著加分——但不得绕过质量门槛。`,
        ...
        '9. literature_fetch_pdf（传 pushId）：下载选中论文 PDF（多源回退）。',
